...
export const TOTAL_LENGTH = data.length;
...
export function getTransactions(params: {
  ...
  const { page, rowsPerPage, filter, sort  } = params;
  return new Promise((resolve) => {
    setTimeout(() => {
      const searchResult = data;
      const filteredResult = filterRows(searchResult, filter);
      const sortedResult = sortRows(filteredResult, sort);
      resolve({
        total: sortedResult.length,
        data: sortedResult.slice((page - 1) * rowsPerPage, page * rowsPerPage),
      });
